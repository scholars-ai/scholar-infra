...
python3 -c 'import json,sys; d=json.load(sys.stdin); run=next(item for item in d["items"] if item["id"]==sys.argv[1]); assert "funnel" in run["summary"]; assert set(run["summary"]["funnel"]) >= {"source_fetch","topic_scout","topic_evaluate","article_write","article_evaluate","human_review"}; assert "total" in run["summary"]; assert run["summary"]["total"]["artifactCount"] >= 1; assert run["summary"]["observability"]["status"] in {"configured","disabled","unavailable"}' "$workflow_run_id" <<<"$workflow_list"
